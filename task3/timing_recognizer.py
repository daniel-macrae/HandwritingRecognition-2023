...
def main(args):
    ...
    for i in range(10):
        for batch in tqdm(test_dataloader):
            ...
            #print(pred_str)
            n_words = sum([len(x.split()) for x in pred_str])
            dur_word = duration/n_words


            times_per_img.append(duration/batch_size)
            times_per_word.append(dur_word)
    ...
    print("            Mean   STD")
    print(f"Per image {np.mean(times_per_img):.3f},  {np.std(times_per_img):.3f}") 
    print(f"Per word  {np.mean(times_per_word):.3f},  {np.std(times_per_word):.3f}")    
